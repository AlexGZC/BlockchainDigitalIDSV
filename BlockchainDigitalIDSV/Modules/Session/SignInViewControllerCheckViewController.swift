...
class SignInViewControllerCheckViewController: UIViewController, NVActivityIndicatorViewable {
    ...
    @IBAction func loginSuccessfull(_ sender: Any) {
        if self.userTexfield.text?.isEmpty ?? true || (self.userTexfield.text?.trimmingCharacters(in: .whitespaces).isEmpty)!{
            ...
            self.startAnimating()
            let url = "http://192.168.137.219/digitalidsv/api.php?mode=users&id=login"
            let params = User.init(username: self.userTexfield.text!, password: self.passwordTextfield.text!)
            GetAPIManager.Post2(url, params: params.loginUser, success: { (response) in
                ...
                if response["message"].description == "Success"{
                    ...
                    let openRegisterData = storyboard.instantiateViewController(withIdentifier: "DataRegisterViewControllerID") as! DataRegisterViewController
                    openRegisterData.secureCode = response["secure_code"].description
                    self.defaults.set(response["secure_code"].description, forKey: defaultsKeys.securecode)
